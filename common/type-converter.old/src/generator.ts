import { TypeInfo, PropertyInfo } from './parser.js';
...
export class PydanticGenerator {
  ...
  public generateModels(types: TypeInfo[]): string {
    const lines: string[] = [];
  ...
  private generateEnum(enumType: TypeInfo): string[] {
    const lines: string[] = [];
  ...
  private generateModel(model: TypeInfo): string[] {
    const lines: string[] = [];
  ...
  private generateProperty(prop: PropertyInfo): string[] {
    const lines: string[] = [];
  ...
  private generateFieldDescription(prop: PropertyInfo): string {
    // 检查是否是自定义类型
  ...
  private generateTypeAlias(alias: TypeInfo): string[] {
    const lines: string[] = [];
