...
from app.schemas.assignment import AssignData, Submit, TestSubmitRequest,SubmitRequest, TestSample, TestSampleCreate, CodeFileInfo, MatrixAnalysisProps
...
class TestAiController:
    ...
    @classmethod
    async def genResolutions(cls, course_id: str, assign_id: str):
        try:
            ...
            content = AI.getResponse(prompt=AIPrompt.RESOLUTION(
                assign_data.title, assign_data.description, assign_data.assignOriginalCode[0].content))

            return BasicAnalysis.resolution(**content)
