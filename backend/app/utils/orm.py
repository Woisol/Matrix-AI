...
class Field:
    ...
    def to_sql_definition(self, column_name: str) -> str:
        """生成 SQL 字段定义"""
        sql_parts = [column_name, self.field_type.value]
        
        if self.max_length and self.field_type in [FieldType.VARCHAR]:
            sql_parts[1] = f"{self.field_type.value}({self.max_length})"
        
        if self.primary_key:
            sql_parts.append("PRIMARY KEY")
            if self.auto_increment:
                sql_parts.append("SERIAL" if self.field_type == FieldType.INT else "BIGSERIAL")
        ...
        if self.default is not None and not self.auto_increment:
            if isinstance(self.default, str):
                ...
                sql_parts.append(f"DEFAULT {str(self.default).upper()}")
            else:
                sql_parts.append(f"DEFAULT {self.default}")
...
class QueryBuilder:
    ...
    def build_insert(self) -> Tuple[str, List[Any]]:
        ...
        values = [self._values[field] for field in fields]
        ...
        for i in range(0, len(self._insert_data), self.batch_size):
            ...
            for j, item in enumerate(batch):
                ...
                for field in fields:
                    ...
                    all_values.append(item.get(field))
